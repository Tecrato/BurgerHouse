...
if ($url[1] === 'get_all') {
    if (!$session->has_permission('categoria_producto', 'consultar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }

    try {
        $modelo = new Categoria_producto(...$_POST);
    ...
    if (!$session->has_permission('categoria_producto', 'agregar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }
    ...
    try {
        ...
        foreach ($_POST['lista'] as $item) {
            ...
            $itemModel = new Categoria_producto(...$item);
    ...
    if ($active !== null && (string)$active === '0') {
        if (!$session->has_permission('categoria_producto', 'eliminar')) {
            make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
        }
    } else if ($active !== null && (string)$active === '1') {
        if (
            !$session->has_permission('Papelera', 'restaurar') &&
            !$session->has_permission('categoria_producto', 'eliminar')
        ) {
            make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
        }
    } else if (!$session->has_permission('categoria_producto', 'editar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }
    ...
    try {
        $modelo = new Categoria_producto(id: $_POST['id']);
    ...
    make_url_error("Accion no valida para categoria_producto.", 404, ajax: true);
}

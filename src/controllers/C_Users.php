...
if ($url[1] === 'get_all') {
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
        $upload_dir = __DIR__ . '/../media/usuarios/';
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
        if (isset($_POST['id'])) {
            $modelo_actual = new Usuario(id: $_POST['id']);
            $usuario_actual = $modelo_actual->search(n: 0, limite: 1);
            if (!empty($usuario_actual) && !empty($usuario_actual[0]['imagen'])) {
                $imagen_anterior = __DIR__ . '/../media/usuarios/' . $usuario_actual[0]['imagen'];

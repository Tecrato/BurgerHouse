...
if (count($url) < 2 || $url[1] === 'view') {
    if (file_exists(__DIR__ . '/../views/V_producto_procesado.php')) {
        include_once __DIR__ . '/../views/V_producto_procesado.php';
    } else {
        make_url_error("No se encontró la vista producto_procesado.php", 404);
    }
    exit;
}

if ($url[1] === 'get_all') {
    if (!$session->has_permission('productos_procesados', 'consultar')) {
    ...
    if (!$session->has_permission('productos_procesados', 'agregar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }
    ...
    if (isset($_FILES['lista']) && is_array($_FILES['lista'])) {
        foreach ($_FILES['lista']['name']['imagen'] as $index => $fileName) {
            if ($fileName && $_FILES['lista']['error']['imagen'][$index] === UPLOAD_ERR_OK) {
                $upload_dir = __DIR__ . '/../media/productos_procesados/';
                ...
                $file_name = uniqid('prod_proc_', true) . '.' . $file_extension;
    ...
    if ($active !== null && (string)$active === '0') {
        if (!$session->has_permission('productos_procesados', 'eliminar')) {
            make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
        }
    } else if ($active !== null && (string)$active === '1') {
        if (
            !$session->has_permission('Papelera', 'restaurar') &&
            !$session->has_permission('productos_procesados', 'eliminar')
        ) {
            make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
        }
    } else if (!$session->has_permission('productos_procesados', 'editar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }

    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
        if (isset($_POST['id'])) {
            $modelo_actual = new ProductoProcesado(id: $_POST['id']);
            $producto_actual = $modelo_actual->search(n: 0, limite: 1);
            if (!empty($producto_actual) && !empty($producto_actual[0]['imagen'])) {
                $imagen_anterior = __DIR__ . '/../media/productos_procesados/' . $producto_actual[0]['imagen'];
    ...
    make_url_error("Accion no valida para producto_procesado.", 404, ajax: true);
}

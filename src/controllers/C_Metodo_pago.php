...
if ($url[1] === 'get_all') {
    ...
    if ($active !== null && (string)$active === '0') {
        if (!$session->has_permission('metodos_pago', 'eliminar')) {
            make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
        }
    } else if ($active !== null && (string)$active === '1') {
        if (
            !$session->has_permission('Papelera', 'restaurar') &&
            !$session->has_permission('metodos_pago', 'eliminar')
        ) {
            make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
        }
    } else if (!$session->has_permission('metodos_pago', 'editar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }
    ...
    try {
        $modelo = new Metodo_pago(id: $_POST['id']);
    ...
    make_url_error("Accion no valida para metodo_pago.", 404, ajax: true);
}

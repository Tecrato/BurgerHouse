...
{
    $db = new Entrada_materia_prima();
    $db->clear();
    $db->__construct(id: $_POST['id']);
    $r = $db->search()[0];
...
function entrada_materia_prima_add_many(...$args)
{
    try {
        ...
        foreach ($lista_entrada as $key => $value) {
            $db = new Entrada_materia_prima();
            $db->clear();
            $db->__construct(id_proveedor: $value["id_proveedor"]);
            $last_id = $db->agregar();
...
function entrada_materia_prima_inventario(...$args)
{
    try {
        ...
        $db->clear();
        $db->__construct();
        echo json_encode($db->consultar_vista("vista_inventario_materia_prima"));
    } catch (Exception $th) {
        echo json_encode(['success' => false, 'message' => $th->getMessage()]);
    }
}

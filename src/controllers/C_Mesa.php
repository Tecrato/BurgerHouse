...
if (count($url) < 2) {
    if (file_exists(__DIR__ . '/../views/V_mesa.php')) {
        ...
        make_url_error("No se encontró la vista mesa.php", 404);
    }
    exit;
}
...
if ($url[1] === 'get_all') {
    ...
    if (!$session->has_permission('mesas', 'modificar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: $ajax);
    }
    $resultado_final = $modelo->actualizar();
    $ajax = true;
} else if ($url[1] === 'delete') {
    if (!$session->has_permission('mesas', 'borrar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: $ajax);
    }
    $resultado_final = $modelo->borrar();
    $ajax = true;
} else {
    make_url_error("Accion no valida para mesa.", 404, ajax: $ajax);
}

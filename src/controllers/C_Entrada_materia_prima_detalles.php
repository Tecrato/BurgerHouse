...
if (count($url) < 2) {
    make_url_error("Accion no valida para entrada_materia_prima_detalles.", 404, ajax: true);
}
...
if ($accion === 'get_all') {
    try {
        $modelo = new Detalle_entrada_materia_prima(...$_POST);
    ...
    try {
        foreach ($_POST['lista'] as $item) {
            ...
            $itemModel = new Detalle_entrada_materia_prima(...$item);
            $itemModel->agregar();
        }
    ...
    try {
        $modelo = new Detalle_entrada_materia_prima(id: $_POST['id']);
    ...
    foreach ($lista as $item) {
        ...
        $itemModel = new Detalle_entrada_materia_prima(id: $id);

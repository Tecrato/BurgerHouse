...
if ($url[1] === 'get_all') {
    ...
    if (!$session->has_permission('Adicionales', 'agregar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }
    ...
    if ($active !== null && (string)$active === '0') {
        if (!$session->has_permission('Adicionales', 'eliminar')) {
            make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
        }
    } else if ($active !== null && (string)$active === '1') {
        if (
            !$session->has_permission('Papelera', 'restaurar') &&
            !$session->has_permission('Adicionales', 'eliminar')
        ) {
            make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
        }
    } else if (!$session->has_permission('Adicionales', 'editar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }
    ...
    make_url_error("Accion no valida para adicionales.", 404, ajax: true);
}

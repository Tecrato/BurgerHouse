...
if (count($url) < 2) {
    make_url_error("Accion no valida para entrada_producto_procesado_pago.", 404, ajax: true);
}
...
if ($accion === 'get_all') {
    try {
        $modelo = new Pago_entrada_producto_procesado(...$_POST);
    ...
    try {
        foreach ($_POST['lista'] as $item) {
            ...
            $itemModel = new Pago_entrada_producto_procesado(...$item);
            $itemModel->agregar();
        }
    ...
    try {
        $modelo = new Pago_entrada_producto_procesado(id: $_POST['id']);
    ...
    foreach ($lista as $item) {
        ...
        $itemModel = new Pago_entrada_producto_procesado(id: $id);

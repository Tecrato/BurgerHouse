...
if (count($url) < 2) {
    make_url_error("Accion no valida para detalle_orden_producto_procesado.", 404, ajax: true);
}
...
if ($accion === 'get_all') {
    try {
        $modelo = new DetalleOrdenProductoProcesado(...$_POST);
    ...
    try {
        foreach ($_POST['lista'] as $item) {
            ...
            $itemModel = new DetalleOrdenProductoProcesado(...$item);
            $itemModel->agregar();
        }
    ...
    try {
        $modelo = new DetalleOrdenProductoProcesado(id: $_POST['id']);
    ...
    foreach ($lista as $item) {
        ...
        $itemModel = new DetalleOrdenProductoProcesado(id: $id);

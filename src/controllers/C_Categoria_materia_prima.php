...
if (count($url) < 2 || $url[1] === 'view') {
    if (file_exists(__DIR__ . '/../views/V_categoria.php')) {
        ...
        make_url_error("No se encontró la vista V_categoria.php", 404);
    }
    exit;
}
...
if ($url[1] === 'get_all') {
    ...
    try {
        $modelo = new Categoria_materia_prima(...$_POST);
    ...
    if ($active !== null && (string)$active === '0') {
        if (!$session->has_permission('categoria_rawmaterial', 'eliminar')) {
            make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
        }
    } else if ($active !== null && (string)$active === '1') {
        if (
            !$session->has_permission('Papelera', 'restaurar') &&
            !$session->has_permission('categoria_rawmaterial', 'eliminar')
        ) {
            make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
        }
    } else if (!$session->has_permission('categoria_rawmaterial', 'editar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }
    ...
    try {
        $modelo = new Categoria_materia_prima(id: $_POST['id']);
    ...
    make_url_error("Accion no valida para categoria_materia_prima.", 404, ajax: true);
}

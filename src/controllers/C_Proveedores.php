...
if (count($url) < 2 || $url[1] === 'view') {
    if (file_exists(__DIR__ . '/../views/supplier.php')) {
        include_once __DIR__ . '/../views/supplier.php';
    } else {
        make_url_error("No se encontró la vista supplier.php", 404);
    }
    exit;
}

if ($url[1] === 'get_all') {
    if (!$session->has_permission('proveedores', 'consultar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }

    try {
        $modelo = new Proveedor(...$_POST);
    ...
    if (!$session->has_permission('proveedores', 'agregar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }
    ...
    if ($active !== null && (string)$active === '0') {
        if (!$session->has_permission('proveedores', 'eliminar')) {
            make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
        }
    } else if ($active !== null && (string)$active === '1') {
        if (
            !$session->has_permission('Papelera', 'restaurar') &&
            !$session->has_permission('proveedores', 'eliminar')
        ) {
            make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
        }
    } else if (!$session->has_permission('proveedores', 'editar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }
    ...
    try {
        $modelo = new Proveedor(id: $_POST['id']);
    ...
    make_url_error("Accion no valida para proveedor.", 404, ajax: true);
}

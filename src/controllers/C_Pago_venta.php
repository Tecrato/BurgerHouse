...
if ($url[1] === 'get_all') {
    ...
    if (!$session->has_permission('pago_venta', 'agregar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }
    ...
    if (!$session->has_permission('pago_venta', 'editar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }
    ...
    if (!$session->has_permission('pago_venta', 'eliminar')) {
        make_url_error("No tienes permiso para acceder a este recurso.", 403, ajax: true);
    }
    ...
    try {
        $modelo = new Pago_venta(id: $_POST['id']);
    ...
    make_url_error("Accion no valida para pago_venta.", 404, ajax: true);
}

...
let n = $(".table_rawmaterial").DataTable({
    ...
    ajax: function (data, callback, settings) {
        ...
        let totalRaw = myfecth("materia_prima/count", {}, { active: 1 }, null, 'POST');
        let total = 0;
        try {
        ...
        myfecth(
            `materia_prima/get_all/${page}/${size}/${settings.aoColumns[data.order[0].column].data}/${data.order[0].dir}`,
            {},
            { active: 1 },
            function (resp) {
                resp = resp.json()

...
class TestUnidades extends BaseTest {
    ...
    public function __construct() {
        parent::__construct();
        $this->testData = ['nombre' => 'Unidad Test ' . date('His'), 'alias' => 'UT' . date('His')];
        $this->testDataEdit = ['nombre' => 'Unidad Editada ' . date('His'), 'alias' => 'UE' . date('His')];
    }
    
    public function run(): void {
        $this->log("=== PRUEBA CRUD UNIDADES ===");
        $this->testCreate();
        $this->testRead();
        $this->testUpdate();
    ...
    private function testCreate(): void {
        ...
        try {
            $this->driver->findElement(WebDriverBy::cssSelector('[data-bs-target="#registrar_unidad"]'))->click();
        ...
        try {
            $this->driver->findElement(WebDriverBy::cssSelector('#enviar_unidad'))->click();
        } catch (NoSuchElementException $e) {
    ...
    private function testUpdate(): void {
        ...
        try {
            $this->driver->findElement(WebDriverBy::cssSelector('#enviar_unidad_editar'))->click();
        } catch (NoSuchElementException $e) {
            $this->driver->getKeyboard()->pressKey(WebDriverKeys::ENTER);
        }
        $this->wait(3);
